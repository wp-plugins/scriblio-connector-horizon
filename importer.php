...
/*
Plugin Name: Scriblio Horizon Connector
...
class ScribHorizon_import { 
	...
	var $importer_name = 'Scriblio Horizon Connector'; 
	...
	function greet() {
		...
		if( isset( $_POST['scrib_horizon-delete'] ) && isset( $_POST['scrib_horizon-sourceprefix'] )){
			$save_prefs = get_option('scriblio-importer-horizon') or array();

			unset( $save_prefs[ str_pad( substr( preg_replace( '/[^a-z0-9]/', '', strtolower( $_POST['scrib_horizon-sourceprefix'] )), 0, 2), 2, 'a', STR_PAD_LEFT ) ] );

			update_option('scriblio-importer-horizon', $save_prefs);
		
		}else if( isset( $_POST['scrib_horizon-sourcename'] )){
			$save_prefs = get_option('scriblio-importer-horizon') or array();

			$prefs['sourcename'] = wp_filter_nohtml_kses( $_POST['scrib_horizon-sourcename'] );
			$prefs['sourceurl'] = sanitize_url( $_POST['scrib_horizon-sourceurl'] );
			$prefs['sourceprefix'] = str_pad( substr( preg_replace( '/[^a-z0-9]/', '', strtolower( $_POST['scrib_horizon-sourceprefix'] )), 0, 2), 2, 'a', STR_PAD_LEFT );
			$prefs['convert_encoding'] = isset( $_POST['scrib_horizon-convert_encoding'] );
			$prefs['bibrange_low'] = absint( $_POST['scrib_horizon-bibrange_low'] );
			$prefs['bibrange_high'] = absint( $_POST['scrib_horizon-bibrange_high'] );
			$prefs['require_import'] = $_POST['scrib_horizon-require_import'];
			$prefs['reject_import'] = $_POST['scrib_horizon-reject_import'];
			$prefs['capitalize_titles'] = isset( $_POST['scrib_horizon-capitalize_titles'] );
			$prefs['require_availability'] = $_POST['scrib_horizon-require_availability'];
			$prefs['reject_availability'] = $_POST['scrib_horizon-reject_availability'];
		...
		$save_prefs = get_option('scriblio-importer-horizon');
		...
		if( count( $save_prefs )){
			...
			foreach ($save_prefs as $id => $data) {
				...
				$title = "<a href='admin.php?import=$this->importer_code&amp;step=1&amp;sourceprefix={$data['sourceprefix']}' title='edit connection settings for {$data['sourcename']}'>{$data['sourceprefix']} : {$data['sourcename']}</a>";
		
				$edit = "<a href='admin.php?import=$this->importer_code&amp;step=1&amp;sourceprefix={$data['sourceprefix']}' title='edit connection settings for {$data['sourcename']}'>edit</a>";
		
				$delete = "<a href='admin.php?import=$this->importer_code&amp;step=2&amp;sourceprefix={$data['sourceprefix']}' title='delete the connection to {$data['sourcename']}'>delete</a>";
		
				$import = "<a href='admin.php?import=$this->importer_code&amp;step=3&amp;sourceprefix={$data['sourceprefix']}' title='import records from Horizon ILS {$data['sourcename']}'>import</a>";
	...
	function settings() {
		$prefs = get_option('scriblio-importer-horizon');
		if( 
...
?>
		...
		<h3><?php _e('Horizon ILS'); ?></h3>

		<table class="form-table">
		<tr valign="top">
		<th scope="row"><?php _e('The name of this connection', 'scrib') ?></th>
		<td>
		<input name="scrib_horizon-sourcename" type="text" id="scrib_horizon-sourcename" value="<?php echo attribute_escape( $prefs['sourcename'] ); ?>" /><br />example: Lincoln University Library Catalogue
		</td>
		</tr>
		
		<tr valign="top">
		<th scope="row"><?php _e('The single record URL template', 'scrib') ?></th>
		<td>
		<input name="scrib_horizon-sourceurl" type="text" id="scrib_horizon-sourceurl" value="<?php echo attribute_escape( $prefs['sourceurl'] ); ?>" /><br />example: http://www.library.lincoln.ac.uk/ipac20/ipac.jsp?profile=ln&ri=6&menu...
~!horizon_test&fullmarc=true&GetXML=true&uri=full=3100001~!%BIBNUMBER%~!0
		</td>
		</tr>
		...
		<td>
		<input name="scrib_horizon-sourceprefix" type="text" id="scrib_horizon-sourceprefix" value="<?php echo attribute_escape( $prefs['sourceprefix'] ); ?>" /><br />
...
<?php if( function_exists( 'mb_convert_encoding' )){ ?>
		<tr>
		<th scope="row" class="th-full"><label for="scrib_horizon-convert_encoding">Convert character encoding</label></th>
		<td><input type="checkbox" name="scrib_horizon-convert_encoding" id="scrib_horizon-convert_encoding" value="1" <?php if( !empty( $prefs['convert_encoding'] )) echo 'CHECKED'; ?> /><br />
<?php
...
<?php } ?>
		...
		<td>
		<input name="scrib_horizon-bibrange_low" type="text" id="scrib_horizon-bibrange_low" value="<?php echo attribute_escape( $prefs['bibrange_low'] ); ?>" /> to <input name="scrib_horizon-bibrange_high" type="text" id="scrib_horizon-bibrange_high" value="<?php echo attribute_escape( $prefs['bibrange_high'] ); ?>" /><br />
		example: 287000 to 600000 (the lowest to highest bibnumber number you&apos;re likely to see in a few years)
		</td>
		</tr>
		...
		<td>
		<input name="scrib_horizon-require_import" type="text" id="scrib_horizon-require_import" value="<?php echo format_to_edit( $prefs['require_import'] ); ?>" /><br />
		...
		<td>
		<input name="scrib_horizon-reject_import" type="text" id="scrib_horizon-reject_import" value="<?php echo format_to_edit( $prefs['reject_import'] ); ?>" /><br />
		...
		<tr>
		<th scope="row" class="th-full"><label for="scrib_horizon-capitalize_titles">Capitalize titles</label></th>
		<td><input type="checkbox" name="scrib_horizon-capitalize_titles" id="scrib_horizon-capitalize_titles" value="1" <?php if( !empty( $prefs['capitalize_titles'] )) echo 'CHECKED'; ?> /></td>
		</tr>
		</table>
		...
		<td>
		<input name="scrib_horizon-require_availability" type="text" id="scrib_horizon-require_availability" value="<?php echo format_to_edit( $prefs['require_availability'] ); ?>" /><br />
		example: My Library Location Name (optional; leave blank to harvest any record)<br />
		uses <a href="http://php.net/strpos">strpos</a> matching rules
		</td>
		</tr>
		...
		<td>
		<input name="scrib_horizon-reject_availability" type="text" id="scrib_horizon-reject_availability" value="<?php echo format_to_edit( $prefs['reject_availability'] ); ?>" /><br />
		example: No Such Record<br />
		uses <a href="http://php.net/strpos">strpos</a> matching rules 
		</td>
		</tr>
...
?>
			...
			<p><?php echo $prefs['sourceurl']; ?></p>

			<form name="myform" id="myform" action="admin.php?import=<?php echo $this->importer_code; ?>" method="post">
	
			<input name="scrib_horizon-sourceprefix" type="hidden" id="scrib_horizon-sourceprefix" value="<?php echo attribute_escape( $prefs['sourceprefix'] ); ?>" />

			<input name="scrib_horizon-delete" type="hidden" id="scrib_horizon-delete" value="1" />
...
<?php
	...
	function get_range( $record_start = FALSE, $record_end = FALSE ){
		global $wpdb, $scrib;
		...
		if( 
			...
			<h3><?php _e('Importing records from:'); ?> <?php echo $prefs['sourceprefix']; ?> : <?php echo $prefs['sourcename']; ?></h3>
			...
			<td>
			<input type="text" name="scrib_horizon-record_start" id="scrib_horizon-record_start" value="<?php echo attribute_escape( $record_start ); ?>" /><br />
			</td>
			</tr>
			...
			<td>
			<input type="text" name="scrib_horizon-record_end" id="scrib_horizon-record_end" value="<?php echo attribute_escape( $record_end ); ?>" />
			</td>
			</tr>
			...
			<label for="scrib_horizon-debug"><input type="checkbox" name="scrib_horizon-debug" id="scrib_horizon-debug" value="1" = /> Debug mode</label>
			</th>
			</tr>
			<tr>
			</table>
...
<?php
	...
	function get_records(){
		...
		if( 
			...
			$interval = 25;
			if( !$_REQUEST['scrib_horizon-record_end'] || ( $_REQUEST['scrib_horizon-record_end'] == $prefs['scrib_horizon-record_start'] ))
				$_REQUEST['scrib_horizon-debug'] = TRUE;
			if( !$_REQUEST['scrib_horizon-record_end'] || ( $_REQUEST['scrib_horizon-record_end'] - $_REQUEST['scrib_horizon-record_start'] < $interval ))
				$interval = $_REQUEST['scrib_horizon-record_end'] - $_REQUEST['scrib_horizon-record_start'];
			if( $_REQUEST['scrib_horizon-record_end'] - $_REQUEST['scrib_horizon-record_start'] < 1 )
				$interval = 0;
			...
			if( !empty( $_REQUEST['scrib_horizon-debug'] )){
				$bibn = (int) $_REQUEST['scrib_horizon-record_start'];
	
				echo '<h3>The Horizon Record:</h3><pre>';			
				echo $this->get_record( $prefs['sourceprefix'], $bibn );
				echo '</pre><h3>The Tags and Display Record:</h3><pre>';
	
				$test_pancake = $this->parse_record( $prefs['sourceprefix'], $this->get_record( $prefs['sourceprefix'], $bibn ), $bibn );
				...
				$count = 0;
				echo "<p>Reading a batch of $interval records from {$prefs['sourcename']}. Please be patient.<br /><br /></p>";
				echo '<ol>';
				for( $bibn = (int) $_REQUEST['scrib_horizon-record_start'] ; $bibn < ( $_REQUEST['scrib_horizon-record_start'] + $interval ) ; $bibn++ ){
					if($record = $this->get_record( $prefs['sourceprefix'] , $bibn )){
						$bibr = $this->parse_record( $prefs['sourceprefix'], $record , $bibn );
				...
				if( $bibn < $_REQUEST['scrib_horizon-record_end'] ){
					$_REQUEST['scrib_horizon-record_start'] = $_REQUEST['scrib_horizon-record_start'] + $interval;
	
					$this->get_range( $_REQUEST['scrib_horizon-record_start'], $_REQUEST['scrib_horizon-record_end'] );
					?>
	...
	function get_record( $prefix, $bibn ){
		$prefs = get_option('scriblio-importer-horizon');
		$prefs = $prefs[ $prefix ];
		if( !is_array( $prefs ))
			return( FALSE );

		// get the MARC view of the record
		$record = wp_remote_get( str_replace( '%BIBNUMBER%', $bibn, $prefs['sourceurl'] ));
		...
		preg_match( '/<fullnonmarc>(.*)<\/fullnonmarc>/i', $record, $fullnonmarc );
		preg_match_all( '/<label>([^<]*)</i', $fullnonmarc[1], $fields );
		preg_match_all( '/<text>([^<]*)</i', $fullnonmarc[1], $values );
		
		$record = '';
		foreach( $fields[1] as $k => $v) // combine the two to get a decent representation of the record
			$record .= $v .' '. ( intval( $v < 10 ) ? ' '. $values[1][$k] : $values[1][$k] ). "\n"; 
...
*/
	...
	function parse_row($lineray){
		...
		foreach($lineray as $element){
			$count[$element{1}]++;
			$elementname = $element{1}.$count[$element{1}];
			$marcrow[$elementname] = trim( substr( $element, 2 ));
		}

		return($marcrow);
	}

	function parse_record( $sourceprefix, &$marcrecord, &$bibn ){
		global $scrib;
		...
		$details[0] = str_replace('LDR', '000', $details[0]);
		...
		foreach($details as $line){		
			...
			//Construct $lineray
			$lineray = array( substr( $line, 0, 3 ), substr( $line, 4, 2 ), substr( $line, 7 ));
			$temp = preg_split( '/\$([a-z]) /i', $lineray[2], -1, PREG_SPLIT_DELIM_CAPTURE );
			if( empty( $temp[0] )){
				unset( $lineray[2] );
				unset( $temp[0] );
				foreach( $temp as $k => $v ){
			...
			// tag 600 - Person
				...
				$marc = array_map( array( 'scrib', 'meditor_sanitize_punctuation' ), $this->parse_row( $lineray ));
...
*/
	...
	function availability( &$post_id, &$sourceid ){
		global $scrib;
		...
		$this->harvest_passive_single_schedule( $prefs['sourceprefix'], $bibn ); // updated the harvested record

		$cache = wp_cache_get( $sourceid , 'scrib_availability' );
		if( !is_array( $cache )){
			
			$raw = wp_remote_get( 'http://'. $prefs['sourceurl'] .'/record='. $bibn );
			if( is_wp_error( $raw ))
				return( '<li class="scrib_availability_horizon">There was an error while connecting to the inventory system. <a href="http://'. $prefs['sourceurl'] .'/record='. $bibn .'">Click here to try for yourself</a>.</li>' );
...
// todo: fix this to deal with the new items array
		...
		{	
			...
			{
				$return .= '<li class="scrib_availability_horizon"><span class="order">'.__($order_status[0]).'</span></li>'."\n";
			}
		}
		// End of Mike D
	...
	function availability_filter( &$content, &$post_id, &$idnumbers ){
		$prefs = get_option('scriblio-importer-horizon');
		...
		foreach( $idnumbers['sourceid'] as $sourceid ){
			if( in_array( substr( $sourceid, 0, 2 ), $connections ))
				$return .= $this->availability( $post_id, $sourceid );
		}

		return( $content . $return );
	}

	function harvest_passive(){
		global $wpdb, $scrib, $bsuite;

		if ( get_option( 'scriblio-importer-horizon-passvimport') > time() || !$bsuite->get_lock( 'scriblio-importer-horizon-passvimport' ) )
			return( TRUE );
		...
		if( $prefs['bibrange_high'] > $record_end )
			$this->harvest_range( $prefs['sourceprefix'], $record_start, $record_end );

		update_option( 'scriblio-importer-horizon-passvimport', time() + 14400 );
	}

	function harvest_passive_single_schedule( $prefix, $bibn ) { 
		global $wpdb, $scrib; 
...
//		wp_clear_scheduled_hook( 'scrib-importer-horizon-harvest-single', 'ak', 999987 , 1000017 );

		wp_schedule_single_event( time() + rand( 60, 585 ) , 'scrib-importer-horizon-harvest-single' , array( $prefs['sourceprefix'] . $bibn ));

		wp_cache_set( $bibr['_sourceid'], time() + 86400, 'scrib_harvested', time() + 86400 );
	} 

	function harvest_passive_single( $sourceid ) {
		global $bsuite; 

		//error_log( "Updating a single record: ". $sourceid );

		if( !$bsuite->get_lock( 'scriblio-importer-horizon-updsingle' )){
			wp_schedule_single_event( time() + rand( 60, 585 ) , 'scrib-importer-horizon-harvest-single' , array( $sourceid ) );
			return( FALSE );
		}
		...
		$this->harvest_range( $prefs['sourceprefix'], $min, $max );
	} 

	function harvest_range( $prefix, $bibn, $bibn_end ) {
		$bibn = absint( $bibn );
		$bibn_end = absint( $bibn_end );

		for( $bibn ; $bibn <= $bibn_end ; $bibn++ )
			$this->parse_record( $prefix, $this->get_record( $prefix , $bibn ) , $bibn );
	} 

	function init(){
		global $bsuite;

		add_filter( 'scrib_availability_excerpt', array( &$this, 'availability_filter' ), 9, 3);
		add_filter( 'scrib_availability_content', array( &$this, 'availability_filter' ), 9, 3);

		add_action('scrib-importer-horizon-harvest-single', array( &$this, 'harvest_passive_single' ));
		add_action('scrib-importer-horizon-harvest-single', 'harvest_passive_single' );

		if( $bsuite->loadavg < get_option( 'bsuite_load_max' )) // only do cron if load is low-ish
			add_filter('bsuite_interval', array( &$this, 'harvest_passive' ));
	}
